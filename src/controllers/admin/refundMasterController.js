...
const refundMasterService = require('../../services/admin/refundMasterService');
...
const refundMasterController = {
    add: async (req, res) => {
        ...
        try {
            ...
            const list = await refundMasterService.getAll();
    ...
    getAll: async (req, res) => {
        try {
            const data = await refundMasterService.getAll();
    ...
    changeStatus: async (req, res) => {
        try {
            const { error, value } = refundMasterValidatedSchema.changeStatusSchema.validate(req.body);

            const data = await refundMasterService.changeStatus(req.params.refundId, value);
            return res.status(200).json({ status: 200, data: data });
        } catch (error) {
            return res.status(500).json({ status: 500, error: error.message });
        }
